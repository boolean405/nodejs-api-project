...
let initialize = async (io, socket) => {
  socket["current_user_id"] = socket.user_data._id;

  await liveUser(socket.id, socket.user_data);
...
let incommingMessage = async (io, socket, data) => {
  ...
  if (db_from_user) {
    if (db_to_user) {
      ...
      if (to_user) {
        let to_user_socket = io.of("/chat").to(to_user.socket_id);
        if (to_user_socket) {
